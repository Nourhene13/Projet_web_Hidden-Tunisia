...
class RecController extends AbstractController
{
    ...
    public function afficheR(RecRepository $rec,PaginatorInterface $paginator,Request $request): Response
    {
        $pagination = $paginator->paginate($rec->findAll(), $request->query->getInt('page', 1),
            1 // items per page
        );
        return $this->render('reclamation/listeR.html.twig', [
            'recs' => $pagination,'rec'=>$rec
        ]);
    }



    #[Route('/afficheRTrieStatue', name: 'afficheRTrieStatue')]
    public function afficheRTrieStatue(RecRepository $rec,PaginatorInterface $paginator,Request $request): Response
    {
        $s=$rec->TrieStatue();
        $pagination = $paginator->paginate($s, $request->query->getInt('page', 1),
            3// items per page
        );
        return $this->render('reclamation/listeR.html.twig', [
        'recs' => $pagination,'rec'=>$rec
